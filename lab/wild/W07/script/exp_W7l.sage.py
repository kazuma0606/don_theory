...
# This file was *autogenerated* from the file exp_W7l.sage
from sage.all_cmdline import *   # import sage library

_sage_const_65 = Integer(65); _sage_const_42 = Integer(42); _sage_const_24 = Integer(24); _sage_const_0p25 = RealNumber('0.25'); _sage_const_0p1 = RealNumber('0.1'); _sage_const_4 = Integer(4); _sage_const_1p0 = RealNumber('1.0'); _sage_const_0p3 = RealNumber('0.3'); _sage_const_2 = Integer(2); _sage_const_1en8 = RealNumber('1e-8'); _sage_const_0p2 = RealNumber('0.2'); _sage_const_0p05 = RealNumber('0.05'); _sage_const_0p5 = RealNumber('0.5'); _sage_const_25 = Integer(25); _sage_const_3p0 = RealNumber('3.0'); _sage_const_0p4 = RealNumber('0.4'); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_1p2 = RealNumber('1.2'); _sage_const_0p9 = RealNumber('0.9'); _sage_const_0p8 = RealNumber('0.8'); _sage_const_0p7 = RealNumber('0.7'); _sage_const_0p6 = RealNumber('0.6'); _sage_const_0p85 = RealNumber('0.85'); _sage_const_6 = Integer(6); _sage_const_30 = Integer(30); _sage_const_1en10 = RealNumber('1e-10'); _sage_const_0p0 = RealNumber('0.0'); _sage_const_3 = Integer(3); _sage_const_10 = Integer(10); _sage_const_20 = Integer(20); _sage_const_50 = Integer(50); _sage_const_100 = Integer(100); _sage_const_150 = Integer(150); _sage_const_18 = Integer(18); _sage_const_8 = Integer(8); _sage_const_5 = Integer(5); _sage_const_0p01 = RealNumber('0.01'); _sage_const_0p03 = RealNumber('0.03'); _sage_const_9 = Integer(9); _sage_const_120 = Integer(120); _sage_const_13 = Integer(13); _sage_const_2p5 = RealNumber('2.5'); _sage_const_2p0 = RealNumber('2.0'); _sage_const_1p8 = RealNumber('1.8'); _sage_const_1p5 = RealNumber('1.5'); _sage_const_2p2 = RealNumber('2.2'); _sage_const_1p05 = RealNumber('1.05'); _sage_const_14 = Integer(14); _sage_const_40 = Integer(40); _sage_const_7p5 = RealNumber('7.5'); _sage_const_1p1 = RealNumber('1.1'); _sage_const_3p5 = RealNumber('3.5'); _sage_const_11p5 = RealNumber('11.5'); _sage_const_12p5 = RealNumber('12.5')#!/usr/bin/env sage
...
for perm in iter_perms(range(_sage_const_1 , K+_sage_const_1 )):
    ...
    # Burau 行列（t=1/2）
    BM   = b.burau_matrix()(t=QQ(_sage_const_1 )/QQ(_sage_const_2 ))
...
for fn in features_check:
    ...
    bar = "█" * int(abs(r_val) * _sage_const_30 )
    sign = "+" if r_val >= _sage_const_0  else "-"
...
def normalize(arr):
    s = arr.std()
    return (arr - arr.mean()) / s if s > _sage_const_1en10  else arr * _sage_const_0p0 
...
for ct, v in ct_stats.items():
    ...
    rr = float(stats.pearsonr(bv, dv)[_sage_const_0 ]) if len(dv) >= _sage_const_3  else _sage_const_0p0 
    ct_info[ct] = {
...
dir_bur = -_sage_const_1p0  if r_bur > _sage_const_0  else +_sage_const_1p0 
dir_cay = -_sage_const_1p0  if r_cay > _sage_const_0  else +_sage_const_1p0 
# λ = |r_cayley| / |r_burau| なら等比重, ここでは比率で自動決定
lam_linear = abs(r_cay) / (abs(r_bur) + _sage_const_1en8 )
...
r_logcay = float(stats.pearsonr(log_cay_arr, dists)[_sage_const_0 ])
dir_logcay = -_sage_const_1p0  if r_logcay > _sage_const_0  else +_sage_const_1p0 
lam_log    = abs(r_logcay) / (abs(r_bur) + _sage_const_1en8 )
...
dir_spec = -_sage_const_1p0  if r_spec > _sage_const_0  else +_sage_const_1p0 
...
dir_frob = -_sage_const_1p0  if r_frob > _sage_const_0  else +_sage_const_1p0 
...
dir_eig = -_sage_const_1p0  if r_eig > _sage_const_0  else +_sage_const_1p0 

def score_C_eigvar(r): return dir_eig * r["eig_var"]

# ── C4: Burau + スペクトル複合 ──────────────────────────────
lam_spec = abs(r_spec) / (abs(r_bur) + _sage_const_1en8 )
...
def ct_prior_score(ct_str):
    ...
    normalized = (md - ct_mean_min) / (ct_mean_max - ct_mean_min + _sage_const_1en8 )
...
ct_burau_stats = {ct: (np.mean(v), np.std(v)+_sage_const_1en8 )
...
def score_D_confidence(r):
    ...
    return abs_r * bur_s + (_sage_const_1p0  - abs_r) * ct_s
...
borda_components = [
    ("burau",     lambda r: r["burau"],    dir_bur > _sage_const_0 ),
    ("cayley",    lambda r: r["cayley"],   dir_cay > _sage_const_0 ),
    ("max_cycle", lambda r: r["max_cycle"],False),   # 小さいほど良い
    ("spec_rad",  lambda r: r["spec_rad"], dir_spec > _sage_const_0 ),
    ("frob",      lambda r: r["frob"],     dir_frob > _sage_const_0 ),
]
...
def threestage_labels(rows_sub, N):
    ...
    for ct, grp in ct_g.items():
        ...
        direc = -_sage_const_1  if rr > _sage_const_0  else +_sage_const_1 
...
print(hdr);  print("  " + "-"*(_sage_const_18  + _sage_const_8 *len(N_vals) + _sage_const_2 ))
...
for fold_idx, (tr_idx, te_idx) in enumerate(kf.split(rows)):
    ...
    r_bur_f  = float(stats.pearsonr(bur_tr, dist_tr)[_sage_const_0 ])
    r_cay_f  = float(stats.pearsonr(cay_tr, dist_tr)[_sage_const_0 ])
    r_lcay_f = float(stats.pearsonr(lcay_tr, dist_tr)[_sage_const_0 ])
    r_spec_f = float(stats.pearsonr(spec_tr, dist_tr)[_sage_const_0 ])
    d_bur_f  = -_sage_const_1p0  if r_bur_f  > _sage_const_0  else +_sage_const_1p0 
    d_cay_f  = -_sage_const_1p0  if r_cay_f  > _sage_const_0  else +_sage_const_1p0 
    d_lcay_f = -_sage_const_1p0  if r_lcay_f > _sage_const_0  else +_sage_const_1p0 
    d_spec_f = -_sage_const_1p0  if r_spec_f > _sage_const_0  else +_sage_const_1p0 
    lam_lin_f = abs(r_cay_f)  / (abs(r_bur_f) + _sage_const_1en8 )
    lam_log_f = abs(r_lcay_f) / (abs(r_bur_f) + _sage_const_1en8 )
    lam_spc_f = abs(r_spec_f) / (abs(r_bur_f) + _sage_const_1en8 )
    ...
    if ct_info_f:
        ...
        cmn, cmx = _sage_const_0p0 , _sage_const_1p0 

    def ct_ps_f(ct_str):
        if ct_str not in ct_info_f: return _sage_const_0p0 
        md = ct_info_f[ct_str]["mean_dist"]
        return -((md - cmn) / (cmx - cmn + _sage_const_1en8 ))

    bur_by_ct_f = defaultdict(list)
    for r in rows_tr: bur_by_ct_f[r["ct_str"]].append(r["burau"])
    ct_bst_f = {ct: (np.mean(v), np.std(v)+_sage_const_1en8 ) for ct, v in bur_by_ct_f.items()}
    ...
    def sc_D(r, idx):
        ct = r["ct_str"]
        if ct not in ct_info_f: return _sage_const_0p0 
        abs_r = abs(ct_info_f[ct]["r"])
        direc = float(ct_info_f[ct]["direction"])
        mu, sig = ct_bst_f.get(ct, (_sage_const_0p0 , _sage_const_1p0 ))
        bur_s = direc * (r["burau"] - mu) / sig
        return abs_r * bur_s + (_sage_const_1p0  - abs_r) * ct_ps_f(ct)

    # Borda on test fold
    borda5_f = make_borda_score_subset(rows_te, [
        ("burau",    lambda r: r["burau"],    d_bur_f  > _sage_const_0 ),
        ("cayley",   lambda r: r["cayley"],   d_cay_f  > _sage_const_0 ),
        ("max_cyc",  lambda r: r["max_cycle"],False),
        ("spec_rad", lambda r: r["spec_rad"], d_spec_f > _sage_const_0 ),
    ...
    for N in N_vals:
        ...
        for name, sfn_f in FOLD_SCORERS.items():
            ranked = sorted(enumerate(rows_te),
                            key=lambda x: sfn_f(x[_sage_const_1 ], x[_sage_const_0 ]), reverse=True)
...
bars = ax.bar(feat_names_plot, feat_corrs, color=colors_bar, alpha=_sage_const_0p85 )
...
ax.set_xticklabels(feat_names_plot, rotation=_sage_const_30 , ha='right')
for b, v in zip(bars, feat_corrs):
    ax.text(b.get_x()+b.get_width()/_sage_const_2 , v+(_sage_const_0p01  if v>=_sage_const_0  else -_sage_const_0p03 ),
            f"{v:.3f}", ha='center', fontsize=_sage_const_8 )
...
ax.legend(handles=leg, fontsize=_sage_const_9 )
plt.tight_layout()
fig.savefig(OUT / "W7l_fig1_correlations.png", dpi=_sage_const_120 , bbox_inches="tight")
plt.close(fig)
...
fig.suptitle("W-7l: Precision@N — All Analytical Approaches (5-fold CV)", fontsize=_sage_const_13 )

palette = {
    "GlobalBurau"    : ("blue",     "o",  "--", _sage_const_2p5 ),
    "3Stg-25%"       : ("green",    "s",  "-",  _sage_const_2p5 ),
    "A:Cayley"       : ("teal",     "^",  "-",  _sage_const_2p0 ),
    "B1:Bur+Cay(r)"  : ("purple",   "D",  "-",  _sage_const_1p8 ),
    "B2:Bur+Cay(log)": ("violet",   "v",  "-",  _sage_const_1p8 ),
    "B3:Bur+Cay(eq)" : ("indigo",   "P",  "-",  _sage_const_1p8 ),
    "C1:SpecRad"     : ("red",      "h",  "-",  _sage_const_1p8 ),
    "C2:Frob"        : ("salmon",   "x",  "-",  _sage_const_1p5 ),
    "C3:EigVar"      : ("tomato",   "+",  "-",  _sage_const_1p5 ),
    "C4:Bur+Spec"    : ("crimson",  "1",  "-",  _sage_const_1p8 ),
    "D:Confidence"   : ("darkorange","*", "-",  _sage_const_2p2 ),
    "E1:Borda(5)"    : ("brown",    "2",  "-",  _sage_const_1p8 ),
    "E2:Borda(2)"    : ("sienna",   "3",  "-",  _sage_const_1p8 ),
}

group_plots = [
    ("Baseline",    ["GlobalBurau","3Stg-25%"],             axes[_sage_const_0 ,_sage_const_0 ]),
    ("A: Cayley",   ["GlobalBurau","3Stg-25%","A:Cayley"],  axes[_sage_const_0 ,_sage_const_1 ]),
    ("B: Reg Burau",["GlobalBurau","3Stg-25%",
                     "B1:Bur+Cay(r)","B2:Bur+Cay(log)","B3:Bur+Cay(eq)"], axes[_sage_const_0 ,_sage_const_2 ]),
    ("C: Spectral", ["GlobalBurau","3Stg-25%",
                     "C1:SpecRad","C2:Frob","C3:EigVar","C4:Bur+Spec"],    axes[_sage_const_1 ,_sage_const_0 ]),
    ("D: Confidence",["GlobalBurau","3Stg-25%","D:Confidence"],            axes[_sage_const_1 ,_sage_const_1 ]),
    ("E: Borda",    ["GlobalBurau","3Stg-25%",
                     "E1:Borda(5)","E2:Borda(2)"],                         axes[_sage_const_1 ,_sage_const_2 ]),
]
for title, keys, ax in group_plots:
    ...
    ax.legend(fontsize=_sage_const_8 ); ax.grid(True, alpha=_sage_const_0p3 ); ax.set_ylim(_sage_const_0 , _sage_const_1p05 )

plt.tight_layout()
fig.savefig(OUT / "W7l_fig2_precision_groups.png", dpi=_sage_const_120 , bbox_inches="tight")
plt.close(fig)
print(f"  fig2 saved: {OUT / 'W7l_fig2_precision_groups.png'}")

# ── 図3: Precision@50 全手法バーチャート ─────────────────────
fig, ax = plt.subplots(figsize=(_sage_const_14 , _sage_const_5 ))
...
bars = ax.bar(range(len(all_keys_ord)), bar_vals, color=bar_col,
              yerr=bar_errs, capsize=_sage_const_3 , alpha=_sage_const_0p85 )
ax.set_xticks(range(len(all_keys_ord)))
ax.set_xticklabels(all_keys_ord, rotation=_sage_const_40 , ha='right', fontsize=_sage_const_9 )
...
ax.axhline(float(np.mean(cv_prec["GlobalBurau"][N_bar])), ls="--",
           color="#2196F3", alpha=_sage_const_0p5 , label="GlobalBurau")
ax.axhline(float(np.mean(cv_prec["3Stg-25%"][N_bar])), ls="--",
           color="#4CAF50", alpha=_sage_const_0p5 , label="3-Stage-25%")
for i, v in enumerate(bar_vals):
    ax.text(i, v+_sage_const_0p01 , f"{v:.3f}", ha='center', fontsize=_sage_const_7p5 )
ax.legend(fontsize=_sage_const_9 ); ax.set_ylim(_sage_const_0 , _sage_const_1p1 )
# セクション区切り
for x, lbl in [(_sage_const_2p5 ,"A"),(_sage_const_3p5 ,"B"),(_sage_const_7p5 ,"C"),(_sage_const_11p5 ,"D"),(_sage_const_12p5 ,"E")]:
    ax.axvline(x, color='gray', ls=':', alpha=_sage_const_0p4 )
plt.tight_layout()
fig.savefig(OUT / "W7l_fig3_bar_summary.png", dpi=_sage_const_120 , bbox_inches="tight")
plt.close(fig)
