...
# This file was *autogenerated* from the file exp_W7m.sage
from sage.all_cmdline import *   # import sage library

_sage_const_65 = Integer(65); _sage_const_42 = Integer(42); _sage_const_24 = Integer(24); _sage_const_0p25 = RealNumber('0.25'); _sage_const_0p1 = RealNumber('0.1'); _sage_const_4 = Integer(4); _sage_const_1p0 = RealNumber('1.0'); _sage_const_0p3 = RealNumber('0.3'); _sage_const_2 = Integer(2); _sage_const_1en8 = RealNumber('1e-8'); _sage_const_0p2 = RealNumber('0.2'); _sage_const_0p05 = RealNumber('0.05'); _sage_const_0p5 = RealNumber('0.5'); _sage_const_25 = Integer(25); _sage_const_3p0 = RealNumber('3.0'); _sage_const_0p4 = RealNumber('0.4'); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_1p2 = RealNumber('1.2'); _sage_const_0p9 = RealNumber('0.9'); _sage_const_0p8 = RealNumber('0.8'); _sage_const_0p7 = RealNumber('0.7'); _sage_const_0p6 = RealNumber('0.6'); _sage_const_0p85 = RealNumber('0.85'); _sage_const_6 = Integer(6); _sage_const_1en5 = RealNumber('1e-5'); _sage_const_40 = Integer(40); _sage_const_1en4 = RealNumber('1e-4'); _sage_const_1en15 = RealNumber('1e-15'); _sage_const_30 = Integer(30); _sage_const_3 = Integer(3); _sage_const_0p0 = RealNumber('0.0'); _sage_const_999p0 = RealNumber('999.0'); _sage_const_10 = Integer(10); _sage_const_20 = Integer(20); _sage_const_50 = Integer(50); _sage_const_100 = Integer(100); _sage_const_150 = Integer(150); _sage_const_5 = Integer(5); _sage_const_8 = Integer(8); _sage_const_0p005 = RealNumber('0.005'); _sage_const_16 = Integer(16); _sage_const_12 = Integer(12); _sage_const_2p0 = RealNumber('2.0'); _sage_const_2p5 = RealNumber('2.5'); _sage_const_1p8 = RealNumber('1.8'); _sage_const_1p5 = RealNumber('1.5'); _sage_const_2p2 = RealNumber('2.2'); _sage_const_1p05 = RealNumber('1.05'); _sage_const_120 = Integer(120); _sage_const_45 = Integer(45); _sage_const_0p01 = RealNumber('0.01'); _sage_const_6p5 = RealNumber('6.5'); _sage_const_9 = Integer(9); _sage_const_1p15 = RealNumber('1.15')#!/usr/bin/env sage
...
op_weights_norm = op_weights / (op_weights.sum() + _sage_const_1en8 )
for i, (w, wn) in enumerate(zip(op_weights, op_weights_norm)):
    bar = "█" * int(wn * _sage_const_40 )
    print(f"  E{i+_sage_const_1 }: ||J_i - I||_F = {w:.4f}  (weight={wn:.4f})  {bar}")
...
def compute_seq_jacobian(order, K, eps=_sage_const_1en4 ):
    p0 = p0_master.copy()
    ...
    return {
        ...
        "jac_entropy": float(-np.sum(np.abs(eigv) * np.log(np.abs(eigv)+_sage_const_1en15 ))),
    }
...
for perm_iter in iter_perms(range(_sage_const_1 , K+_sage_const_1 )):
    ...
    order= [x-_sage_const_1  for x in perm_iter]   # 0-indexed
    ...
    opw_binary  = -float(sum(op_weights_norm[i]*(_sage_const_1  if inv_perm[i]!=i else _sage_const_0 ) for i in range(K)))
    opw_squared = -float(sum(op_weights_norm[i]*(inv_perm[i]-i)**_sage_const_2  for i in range(K)))
    ...
    rows.append({
        ...
        "ct_str"      : str(ct),
        "first_op"    : int(order[_sage_const_0 ]),  # どの演算子が最初か
        "pos_E1"      : int(inv_perm[_sage_const_0 ]),  # E1が何番目に適用されるか
        # 方向1
...
for fn in new_feats:
    ...
    bar_len = int(abs(rv)*_sage_const_30 )
    sign = "+" if rv >= _sage_const_0  else "-"
...
for fop in sorted(coset_groups.keys()):
    ...
    rv  = float(stats.pearsonr(bv, dv)[_sage_const_0 ]) if len(grp) >= _sage_const_3  else _sage_const_0p0 
    print(f"  E{fop+_sage_const_1 } first: n={len(grp):3d}  mean_dist={dv.mean():.3f}  "
          f"Burau_r={rv:+.3f}")
...
def make_scores_from_subset(rows_sub):
    ...
    # グローバル方向
    r_bur = float(stats.pearsonr(bur, dv)[_sage_const_0 ]) if n>=_sage_const_3  else _sage_const_0p0 
    d_bur = -_sage_const_1p0  if r_bur > _sage_const_0  else +_sage_const_1p0 
    ...
    for jk in jac_keys:
        ...
        rv = float(stats.pearsonr(jv, dv)[_sage_const_0 ]) if n>=_sage_const_3  else _sage_const_0p0 
        jac_dirs[jk] = (-_sage_const_1p0  if rv > _sage_const_0  else +_sage_const_1p0 , float(abs(rv)))

    # OPW 方向
    for ok in ["opw_linear","opw_binary","opw_squared"]:
        ov = np.array([float(r[ok]) for r in rows_sub])
        rv = float(stats.pearsonr(ov, dv)[_sage_const_0 ]) if n>=_sage_const_3  else _sage_const_0p0 
        jac_dirs[ok] = (-_sage_const_1p0  if rv > _sage_const_0  else +_sage_const_1p0 , float(abs(rv)))

    # pos_E1 方向
    pe1 = np.array([float(r["pos_E1"]) for r in rows_sub])
    rv_pe1 = float(stats.pearsonr(pe1, dv)[_sage_const_0 ]) if n>=_sage_const_3  else _sage_const_0p0 
    d_pe1 = -_sage_const_1p0  if rv_pe1 > _sage_const_0  else +_sage_const_1p0 
    ...
    ct_br_stats  = {ct: (float(np.mean(v)), float(np.std(v))+_sage_const_1en8 )
                    for ct, v in ct_burau_loc.items()}
    ct_r_loc     = {}
    for ct in ct_stats_loc:
        bv2 = np.array(ct_burau_loc[ct])
        dv2 = np.array(ct_stats_loc[ct])
        ct_r_loc[ct] = float(stats.pearsonr(bv2, dv2)[_sage_const_0 ]) if len(bv2)>=_sage_const_3  else _sage_const_0p0 
...
def apply_scores(rows_sub, rows_query, stats_dict, fit_fourier=True):
    ...
    def norm_arr(arr):
        s = arr.std(); return (arr-arr.mean())/s if s>_sage_const_1en8  else arr*_sage_const_0p0 
    bur_n_q  = norm_arr(bur_q)
    jt_q     = np.array([float(r["jac_trace"]) for r in rows_query])
    lam_jt   = sd["jac_dirs"]["jac_trace"][_sage_const_1 ] / (abs(sd["r_bur"])+_sage_const_1en8 )
    scores["C5:Bur+Jac"]    = (sd["d_bur"] * norm_arr(bur_q)
                                + lam_jt * d_jt * norm_arr(jt_q))

    # ── D: Coset フィルタスコア ───────────────────────────────────
    # mean_dist の小さい first_op を高スコアに
    co_mean_q = np.array([sd["co_mean"].get(r["first_op"], _sage_const_999p0 ) for r in rows_query])
    scores["D:Coset_prior"] = -co_mean_q

    # ── D2: Coset フィルタ + Burau (後段は ranking で処理) ────────
    # → スコアとしては Coset_prior × (1 + Burau_norm / large_const) で近似
    scores["D2:Coset+Bur"]  = -co_mean_q + _sage_const_0p1  * sd["d_bur"] * norm_arr(bur_q)

    # ── E: Fourier (標準表現行列で線形回帰) ─────────────────────
    if fit_fourier and sd["X_srm"] is not None and len(sd["X_srm"]) >= _sage_const_10 :
        lr = LinearRegression()
...
def threestage_then_score(rows_sub, score_name, all_scores, N):
    ...
    sc_kept.sort(key=lambda x: -x[_sage_const_1 ])
...
def threestage_orig(rows_sub, N):
    ...
    for ct, grp in ct_g.items():
        ...
        n_s = int(max(_sage_const_1 , round(N*len(grp)/len(kept))))
...
for sec_title, keys in SECTIONS:
    print(f"\n  {sec_title}")
    print("  " + "-"*(_sage_const_20  + _sage_const_8 *len(N_vals) + _sage_const_2 ))
    for key in keys:
        line = f"  {key:<20}|"
        for N in N_vals:
            vals = cv_prec[key][N]
        ...
        v50 = np.mean(cv_prec[key][_sage_const_50 ])
        ref = np.mean(cv_prec["3Stg(orig)"][_sage_const_50 ])
        tag = " ★" if v50 > ref + _sage_const_0p005  else ""
        print(line + tag)
...
fig.suptitle("W-7m: New Analytical Approaches — Precision@N (5-fold CV)", fontsize=_sage_const_12 )
...
palettes = {
    ...
    "3Stg(orig)"     : ("green",      "s",  "-",  _sage_const_2p5 ),
    "B1:pos_E1"      : ("teal",       "^",  "-",  _sage_const_1p8 ),
    "B2:OPW_linear"  : ("darkcyan",   "v",  "-",  _sage_const_1p8 ),
    "B3:OPW_sq"      : ("cyan",       "<",  "-",  _sage_const_1p5 ),
    "C1:Jac_trace"   : ("red",        "D",  "-",  _sage_const_1p8 ),
    "C2:Jac_srad"    : ("salmon",     "x",  "-",  _sage_const_1p5 ),
    "C3:Jac_frob"    : ("tomato",     "+",  "-",  _sage_const_1p8 ),
    "C4:Jac_ent"     : ("crimson",    "*",  "-",  _sage_const_2p0 ),
    "C5:Bur+Jac"     : ("darkred",    "P",  "-",  _sage_const_1p8 ),
    "D:Coset_prior"  : ("purple",     "h",  "-",  _sage_const_1p8 ),
    "D2:Coset+Bur"   : ("violet",     "1",  "-",  _sage_const_1p5 ),
    "E:Fourier"      : ("darkorange", "2",  "-",  _sage_const_2p0 ),
    "3Stg+Jac_trace" : ("firebrick",  "D",  "-",  _sage_const_2p2 ),
    "3Stg+Jac_frob"  : ("orangered",  "P",  "-",  _sage_const_2p0 ),
    "3Stg+Jac_ent"   : ("darkred",    "*",  "-",  _sage_const_2p2 ),
    "3Stg+pos_E1"    : ("teal",       "^",  "-",  _sage_const_2p0 ),
    "3Stg+OPW"       : ("darkcyan",   "v",  "-",  _sage_const_2p0 ),
    "3Stg+Fourier"   : ("chocolate",  "2",  "-",  _sage_const_2p0 ),
}

for ax, key_list, title in [
    (axes[_sage_const_0 ], key_methods_left,  "Individual Strategies"),
    (axes[_sage_const_1 ], key_methods_right, "3-Stage × New Score (Combinations)"),
]:
    for key in key_list:
    ...
    ax.set_title(title); ax.legend(fontsize=_sage_const_8 )
    ax.grid(True, alpha=_sage_const_0p3 ); ax.set_ylim(_sage_const_0 , _sage_const_1p05 )

plt.tight_layout()
fig.savefig(OUT / "W7m_fig1_precision.png", dpi=_sage_const_120 , bbox_inches="tight")
plt.close(fig)
print(f"  fig1 saved: {OUT / 'W7m_fig1_precision.png'}")

# ── 図2: Precision@50 バーチャート（全手法） ─────────────────────
fig, ax = plt.subplots(figsize=(_sage_const_16 , _sage_const_5 ))
...
bar_c = []
for sec_title, keys in SECTIONS:
    sc_list = sec_colors.get(sec_title.replace("── ",""), ["#888"]*_sage_const_10 )
    bar_c.extend(sc_list[:len(keys)])

bars = ax.bar(range(len(all_keys_bar)), bar_vals, color=bar_c[:len(all_keys_bar)],
              yerr=bar_errs, capsize=_sage_const_2 , alpha=_sage_const_0p85 )
ax.set_xticks(range(len(all_keys_bar)))
ax.set_xticklabels(all_keys_bar, rotation=_sage_const_45 , ha='right', fontsize=_sage_const_8 )
...
ax.axhline(ref_3stg, ls="-",  color="#4CAF50", alpha=_sage_const_0p7 , lw=_sage_const_1p5 , label=f"3-Stage({ref_3stg:.3f})")
ax.axhline(ref_gb,   ls="--", color="#2196F3", alpha=_sage_const_0p7 , lw=_sage_const_1p5 , label=f"GlobalBurau({ref_gb:.3f})")
for i, v in enumerate(bar_vals):
    ax.text(i, v+_sage_const_0p01 , f"{v:.3f}", ha='center', fontsize=_sage_const_6p5 , rotation=_sage_const_45 )
ax.legend(fontsize=_sage_const_9 ); ax.set_ylim(_sage_const_0 , _sage_const_1p15 )
plt.tight_layout()
fig.savefig(OUT / "W7m_fig2_bar.png", dpi=_sage_const_120 , bbox_inches="tight")
plt.close(fig)
...
fig.suptitle("W-7m: Jacobian Features vs Landscape Distance", fontsize=_sage_const_12 )
...
for ax, (feat, score_name) in zip(axes, jac_plot):
    ...
    dv   = dists
    cv_r = corr_new.get(feat, _sage_const_0p0 )
    colors_scatter = [ct_color_map.get(r["ct_str"], "#888") for r in rows]
    ax.scatter(fv, dv, c=colors_scatter, alpha=_sage_const_0p4 , s=_sage_const_8 )
    ax.set_xlabel(feat); ax.set_ylabel("dist")
    p50 = float(np.mean(cv_prec[score_name][_sage_const_50 ]))
    ax.set_title(f"{feat}\nr={cv_r:.3f}  P@50={p50:.3f}")
    ax.grid(True, alpha=_sage_const_0p2 )

plt.tight_layout()
fig.savefig(OUT / "W7m_fig3_jac_scatter.png", dpi=_sage_const_120 , bbox_inches="tight")
plt.close(fig)
...
for rank, (key, val) in enumerate(all_results, _sage_const_1 ):
    bar = "█" * int(val * _sage_const_30 )
    beat = " ★★★" if val > ref_3stg + _sage_const_0p005  else (" ★" if val > ref_gb + _sage_const_0p005  else "")
...
if val_best > val_3stg:
    ...
    print(f"  → 差 = {(val_3stg-val_best)*_sage_const_100 :.1f}pt  (3-Stageが依然優位)")
...
for jk in ["jac_trace","jac_frob","jac_entropy"]:
    rv = corr_new.get(jk, _sage_const_0p0 )
